...
data = gateio.fetch_ohlcv(symbol='BTC/USDT',timeframe='4h', limit=500,since=since_time)
...
if (lowArray[-1] <= lowerband[-1]):
    sendMail("BTC/USDT触4小时BL下沿：" + str(closeArray[-1]), "BTC/USDT触4小时BL下沿：" + str(closeArray[-1]))
if (highArray[-1] >= upperband[-1]):
    sendMail("BTC/USDT触4小时BL上沿：" + str(closeArray[-1]), "BTC/USDT触4小时BL上沿：" + str(closeArray[-1]))
