...
data_history = ts.get_k_data("600570", ktype = '60')
...
#sendMail("【恒生电子】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]), "【恒生电子】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]))
if ((lowArray[-1] - lowerband[-1])/lowArray[-1] <= 0.01 ):
     sendMail("【恒生电子】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]), "【恒生电子】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]))
if (highArray[-1] >= upperband[-1]):
     sendMail("【恒生电子】触发60分钟布林线上沿,当前价格：" + str(closeArray[-1]), "【恒生电子】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]))
