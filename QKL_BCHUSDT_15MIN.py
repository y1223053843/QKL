...
data = gateio.fetch_ohlcv(symbol='BCH/USDT',timeframe='15m', limit=500,since=since_time)
...
if (lowArray[-1] <= lowerband[-1]):
    sendMail("【BCH/USDT】触15分BL下沿：" + str(closeArray[-1]), "【BCH/USDT】触15分BL下沿：" + str(closeArray[-1]))
if (highArray[-1] >= upperband[-1]):
    sendMail("【BCH/USDT】触15分BL上沿：" + str(closeArray[-1]), "【BCH/USDT】触15分BL上沿：" + str(closeArray[-1]))
