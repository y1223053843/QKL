...
data_history = ts.get_k_data("300750", ktype = '60')
...
#sendMail("【宁德时代】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]), "【宁德时代】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]))
if ((lowArray[-1] - lowerband[-1])/lowArray[-1] <= 0.01 ):
     sendMail("【宁德时代】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]), "【宁德时代】触发60分钟布林线下沿,当前价格：" + str(closeArray[-1]))
if (highArray[-1] >= upperband[-1]):
     sendMail("【宁德时代】触发60分钟布林线上沿,当前价格：" + str(closeArray[-1]), "【宁德时代】触发60分钟布林线上沿,当前价格：" + str(closeArray[-1]))
