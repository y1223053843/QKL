...
data = gateio.fetch_ohlcv(symbol='BCH/USDT',timeframe='4h', limit=500,since=since_time)
...
if (lowArray[-1] <= lowerband[-1]):
    sendMail("【BCH/USDT】触4小时BL下沿：" + str(closeArray[-1]), "【BCH/USDT】触4小时BL下沿：" + str(closeArray[-1]))
if (highArray[-1] >= upperband[-1]):
    sendMail("【BCH/USDT】触4小时BL上沿：" + str(closeArray[-1]), "【BCH/USDT】触4小时BL上沿：" + str(closeArray[-1]))
