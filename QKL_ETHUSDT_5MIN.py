...
data = gateio.fetch_ohlcv(symbol='ETH/USDT',timeframe='5m', limit=500,since=since_time)
...
#sendMail("【ETH/USDT】触发5分钟布林线下沿,当前价格：" + str(closeArray[-1]), "【ETH/USDT】触发5分钟布林线下沿,当前价格：" + str(closeArray[-1]))
if (lowArray[-1] <= lowerband[-1]):
    sendMail("【ETH/USDT】触5分BL下沿：" + str(closeArray[-1]), "【ETH/USDT】触5分BL下沿：" + str(closeArray[-1]))
if (highArray[-1] >= upperband[-1]):
    sendMail("【ETH/USDT】触5分BL上沿：" + str(closeArray[-1]), "【ETH/USDT】触5分BL上沿：" + str(closeArray[-1]))
